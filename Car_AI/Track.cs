...
using static Car_AI.Extensions;
    ...
    public class Track
    {
        ...
        public void GenerateNew(int num)
        {
            ...
            for (int i = 0; i < num; ++i)
            {
                for (int j = 0; j < Game1.maxcars; ++j)
                {
                    cars[j].Add(new Car(startpos[j], j, i));
        ...
        public void Draw(SpriteBatch spritebatch)
        {
            ...
            float bestDist = -9999.0f;
            float bestdrivingtime = 9999999;
            bool IsFinished = cars[0].Exists(x => x.State == Car.FINISHED);
            if (IsFinished)
            {
                Car[] curcars = cars[0].Where(x => x.State == Car.FINISHED).ToArray();
                for (int i = 0; i < curcars.Length; ++i)
                {
                    if (curcars[i].driving_time < bestdrivingtime)
                    {
                ...
                bestID = cars[0].IndexOf(curcars[bestID]);
            }
            else
            {
                for (int i = 0; i < cars[0].Count; ++i)
                {
                    if (cars[0][i].total_dist > bestDist)
                    {
                        bestDist = cars[0][i].total_dist;
                        bestID = i;
                    }
                }
            }
